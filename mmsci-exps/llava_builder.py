...
from transformers import AutoProcessor, AutoModelForVisualQuestionAnswering, \
    Blip2Processor, Blip2ForConditionalGeneration, LlavaForConditionalGeneration, \
    Kosmos2ForConditionalGeneration, LlavaNextProcessor, LlavaNextForConditionalGeneration, \
...
def load_model(model_name):
    ...
    if model_name == 'blip2':
        ...
        processor = AutoTokenizer.from_pretrained(models[model_name], trust_remote_code=True)
        model = AutoModelForCausalLM.from_pretrained(models[model_name], device_map="cuda", trust_remote_code=True).eval()
        model.generation_config = GenerationConfig.from_pretrained(models[model_name], trust_remote_code=True)
    elif model_name == 'llava-next-mistral':
        processor = LlavaNextProcessor.from_pretrained(models[model_name])
        model = LlavaNextForConditionalGeneration.from_pretrained(models[model_name]) 
    elif model_name == 'llava-next' or model_name.startswith('llava-ours') or "mmsci" in model_name:
